...
const grammar = ref(props.grammar);
const langGrammar = ref();
...
const filteredThemes = computed(() => {
  ...
  return themes.filter((t) => t.displayName.toLowerCase().includes(searchTerm));
});
...
async function run(fetchInput = true) {
  ...
  try {
    const themeObject = await import(
      `../node_modules/tm-themes/themes/${theme.value}.json`
    ).then((m) => m.default);
...
function openGrammar(name = grammar.value) {
  openFile(`tm-grammars/grammars/${name}.json`);
}

function openTheme() {
  openFile(`tm-themes/themes/${theme.value}.json`);
}
      ...
      <div h-full of-auto flex="~ col gap-4">
        ...
        <div h-full of-auto flex="~ col" border="~ base rounded">
          <button
            ...
            text-left
            :class="
              g.name === grammar ? 'bg-active text-primary' : 'text-faded'
            "
      ...
      <div flex="~ col gap-4" of-auto>
        ...
        >
          <div grid="~ cols-[max-content_1fr] gap-x-3" flex-auto>
            <div text-right op50>Grammar</div>
            <div>
              <button text-left @click="openGrammar()">
                <code>{{ grammar }}</code>
              </button>
              <div
                v-if="embedded.length < 15"
                flex="~ col"
                ml-2
                border="l base"
              >
            ...
            <div text-right op50>Theme</div>
            <button text-left @click="openTheme()">
              <code>{{ theme }}</code>
            </button>
            <div text-right op50>Sample</div>
            <button text-left @click="openSample()">
              <code>{{ grammar }}.sample</code>
            </button>
            <template v-if="grammarObject?.aliases?.length">
              <div text-right op50>Aliases</div>
              <div flex="~ gap-2">
