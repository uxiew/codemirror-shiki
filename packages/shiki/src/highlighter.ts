...
export class ShikiHighlighter extends Base {
    ...
    */
    highlight(doc: Text, from: number, to: number, buildDeco: (from: number, to: number, mark: Decoration) => void) {
        if (!this.internal) {
        ...
        if (!validTheme) {
            console.warn(`highlight: theme '${themeAlias}' not found in themes config`)
            return
        }
        ...
        // Ensure the correct theme is active for tokenizeLine to produce colors
            ...
            if (this.grammarStateCache.has(stateLine)) {
                ...
                if (nearest < stateLine) {
                    ...
                    for (let i = nearest + 1; i <= stateLine; i++) {
                        ...
                        const result = grammar.tokenizeLine2(lineContent, bridgeState as any)
        ...
        for (let i = startLine; i <= endLine; i++) {
            ...
            for (let j = 0; j < len; j++) {
                ...
                // If foregroundId is 0, use colorMap[1] as fallback, then defaultColor
                const color = colorMap[foregroundId] || colorMap[1] || defaultColor || '#000'
                ...
                if (tokenFrom < tokenTo) { // Only nonempty tokens
                    buildDeco(tokenFrom, tokenTo, Decoration.mark({
                        tagName: 'span',
                        attributes: {
