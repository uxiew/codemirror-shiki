...
function generateLargeCode(lines: number, language: 'javascript' | 'typescript' = 'javascript'): string {
    const templates = {
        javascript: [
            'const variable_{i} = {i};',
...
function measurePerf<T>(name: string, fn: () => T): { result: T; perf: PerfResult } {
...
async function measurePerfAsync<T>(name: string, fn: () => Promise<T>): Promise<{ result: T; perf: PerfResult }> {
    const start = performance.now()
...
describe('Performance: Large File Rendering', () => {
    ...
    beforeAll(async () => {
        ...
        const { perf, result } = await measurePerfAsync('Shiki Initialization', async () => {
    ...
    describe('Initial Render Performance', () => {
        ...
        it('should render medium file (1000 lines) within threshold', async () => {
            ...
            console.log(`[Perf] ${perf.name}: ${perf.duration.toFixed(2)}ms, Lines: ${MEDIUM_FILE_LINES}`)
        ...
        it('should render large file (5000 lines) within acceptable time', async () => {
            ...
            console.log(`[Perf] ${perf.name}: ${perf.duration.toFixed(2)}ms, Lines: ${LARGE_FILE_LINES}`)
    ...
    describe('Incremental Update Performance', () => {
        ...
        it('should update middle of file without re-parsing entire document', async () => {
            ...
            console.log(`[Perf] ${perf.name}: ${perf.duration.toFixed(2)}ms (at position ${middlePos})`)
        ...
        it('should handle rapid typing without lag', async () => {
            ...
            const typingText = 'const rapidTypingTest = "This is a test string for performance";'
            ...
            console.log(`[Perf] Rapid Typing: avg=${avgDuration.toFixed(2)}ms, max=${maxDuration.toFixed(2)}ms, chars=${typingText.length}`)
    ...
    describe('Memory Efficiency', () => {
        it('should not leak memory on repeated updates', async () => {
            ...
            console.log(`[Perf] Memory: initial=${(initialMemory / 1024 / 1024).toFixed(2)}MB, final=${(finalMemory / 1024 / 1024).toFixed(2)}MB, increase=${memoryIncrease.toFixed(2)}MB`)
    ...
    describe('Grammar State Caching', () => {
        it('should cache grammar states for efficient re-highlighting', async () => {
            ...
            console.log(`[Perf] Cold scroll: ${coldPerf.duration.toFixed(2)}ms, Warm scroll: ${warmPerf.duration.toFixed(2)}ms`)
...
describe('Configuration: defaultColor', () => {
    ...
    it('should throw error when defaultColor points to non-existent theme key', async () => {
        // This tests that invalid configuration is caught early
        // 'light' is not a key in themes, so initialization should throw
        await expect(shikiToCodeMirror({
            lang: 'javascript',
            theme: 'nord',
            themes: { nord: 'nord' },  // Only 'nord' key exists
            defaultColor: 'light',     // 'light' key doesn't exist in themes
        })).rejects.toThrow("'light' theme is not registered!")
    })
})
